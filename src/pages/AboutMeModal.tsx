...
export const InfoItem: React.FC<{ info: PersonalInfo }> = ({ info }) => {
  ...
  switch (key) {
    case 'E-mail':
      content = <a href={`mailto:${value}`} className="hover:text-custom-gold">{value}</a>;
      break;
    case 'LinkedIn':
    case 'GitHub':
    case 'Portfólio':
      {
        content = <a href={value} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1">{value} <ExternalLink size={14} /></a>;
        break;
      }
    default:
...
export const PersonalInfos: React.FC<{ personal: PersonalInfo[] }> = React.memo(({ personal }) => (
  <div>
    <h3 className="text-xl font-bold text-white mb-6 uppercase tracking-wider text-left">Informações pessoais</h3>
    <div className="grid grid-cols-2 gap-y-4 gap-x-8">
      {personal.map((info) => (
        <div key={info.key} className={info.key === 'Idiomas' ? 'col-span-2' : ''}>
          <InfoItem info={info} />
        </div>
      ))}
    </div>
    <div className="mt-10 flex justify-start">
      <a
        href="/Guilherme-dos-Anjos-Macedo-CV-Desenvolvedor-Full-Stack.pdf"
        download="Guilherme-dos-Anjos-Macedo-CV-Desenvolvedor-Full-Stack.pdf"
...
export const ExperienceAndEducation: React.FC<{ timeline: TimelineItemType[] }> = React.memo(({ timeline }) => {
  ...
  return (
    <div>
      <SectionTitle>Experiência e formação</SectionTitle>
